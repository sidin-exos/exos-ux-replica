...
const SNAKE_TO_CAMEL: Record<string, keyof DashboardData> = {
  ...
  npv_waterfall: 'npvWaterfall',
  ifrs16_impact: 'ifrs16Impact',
};
...
function extractFromEnvelopeRaw(rawString: string): DashboardData | null {
  ...
  if (group === 'A') {
    ...
    if (validCapexOptions.length >= 1) {
      ...
      if (validSens.length > 0) {
        ...
        // (best collapses to 0 via Math.max, worst displays as negative).
  ...
  // ── S6 Predictive Budgeting & Forecasting (mirror of edge extractor).
 ...
 */
// Convert slug-style headings ("## capex-vs-opex") into readable titles
// ("## CAPEX vs OPEX"). Preserves human-written headings untouched.
